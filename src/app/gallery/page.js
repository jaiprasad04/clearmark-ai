  ...
  const fetchCreations = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setCreations(Array.isArray(data) ? data.filter((c) => c.resultImage) : []);
      }
    } catch {}
    setLoading(false);
  };
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-zinc-950 p-8">
        <div className="text-center max-w-sm">
          ...
          <h2 className="text-base font-bold text-zinc-200">Sign in to view Gallery</h2>
          <p className="text-xs text-zinc-500 mt-2">Your watermark-removed images will appear here.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-zinc-950 py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-xs text-zinc-500 mt-1">
            All your watermark-removed images — ready to download or delete.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <div className="h-20 w-20 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center mx-auto mb-5">
              <FaEraser className="text-3xl text-zinc-700" />
            </div>
            <h3 className="text-sm font-bold text-zinc-300">No results yet</h3>
            <p className="text-xs text-zinc-600 mt-2">Head to the Studio to remove your first watermark.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {creations.map((c) => (
              <div
                key={c.id}
                className="group bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-indigo-500/40 transition-all hover:shadow-xl hover:shadow-indigo-500/5"
              >
                <div
                  className="relative aspect-square bg-zinc-800 cursor-pointer overflow-hidden"
                  onClick={() => { setSelected(c); setCompareMode("result"); }}
                >
                  ...
                  {c.inputImage && (
                    <div className="absolute bottom-2 left-2 h-10 w-10 rounded-md border border-zinc-600 overflow-hidden shadow-lg bg-zinc-800">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={c.inputImage} alt="Original" className="w-full h-full object-cover opacity-70" />
                    </div>
                  )}
                </div>
                <div className="p-3 flex items-center justify-between gap-2">
                  <span className="text-[10px] text-zinc-500 font-medium truncate">
                    {new Date(c.createTime).toLocaleDateString(undefined, { month: "short", day: "numeric" })}
                  </span>
                  <div className="flex gap-1.5">
                    <button
                      onClick={() => handleDownload(c.resultImage, c.id)}
                      className="p-1.5 rounded-lg bg-zinc-800 hover:bg-indigo-600 text-zinc-400 hover:text-white transition-all cursor-pointer"
                      title="Download"
                    >
      ...
      {selected && (
        ...
        >
          <div
            className="bg-zinc-900 border border-zinc-700 rounded-2xl max-w-3xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl"
          ...
          >
            ...
            <div className="flex-1 overflow-auto flex items-center justify-center bg-zinc-950 p-4 min-h-[300px]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={compareMode === "result" ? selected.resultImage : selected.inputImage}
            ...
            <div className="px-5 py-4 border-t border-zinc-800 flex items-center gap-3">
              <p className="text-[10px] text-zinc-500 flex-1 truncate">{selected.prompt?.slice(0, 80)}…</p>
              <button
                onClick={() => handleDownload(selected.resultImage, selected.id)}
                className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white rounded-xl text-xs font-bold cursor-pointer shadow-lg shadow-indigo-500/20 transition-all"
              >
                <FaDownload />
                Download HD
              </button>
              <button
                ...
                className="px-3 py-2.5 bg-zinc-800 hover:bg-red-900/30 hover:text-red-400 border border-zinc-700 hover:border-red-500/30 text-zinc-400 rounded-xl text-xs font-bold transition-all cursor-pointer"
                title="Delete"
              >
                <FaTrashAlt />
              </button>
